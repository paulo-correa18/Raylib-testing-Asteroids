...
void shipRotate(Ship *S, float deltaTime)
{
  ...
  {
    S->rotation -= SHIP_ROTATION_SPEED * deltaTime;
  }
  ...
  S->direction.x = cosf(DEG2RAD * (S->rotation + 90.0f));
  S->direction.y = sinf(DEG2RAD * (S->rotation + 90.0f));
}
...
void shipDraw(Ship *S)
{
  ...
  float rotationRad = DEG2RAD * S->rotation;
  ...
  if (IsKeyDown(KEY_UP))
  {
    // Ponto central traseiro da nave para base do fogo
    ...
    // Desenhar o fogo com cor avermelhada
